...
curl -XDELETE -k -u "$ELASTIC_USER:$ELASTIC_PASSWORD" "$ELASTIC_HOST:$ELASTIC_PORT/$INDEX_NAME" -H 'Content-Type: application/json'
echo
...
while curl -XGET  -k -u "$ELASTIC_USER:$ELASTIC_PASSWORD" "$ELASTIC_HOST:$ELASTIC_PORT/$INDEX_NAME" | grep '"status" : 200'; do
    sleep 1
...
curl -XPUT -k -u "$ELASTIC_USER:$ELASTIC_PASSWORD" "$ELASTIC_HOST:$ELASTIC_PORT/$INDEX_NAME/?format=yaml" -H 'Content-Type: application/json' -d @$CONFIG_DIR/settings.json
echo


echo
echo "ADDING ${INDEX_TO_COLLECTION_MAP[$INDEX_NAME]} DOC TYPE MAPPING"
curl -XPUT -k -u "$ELASTIC_USER:$ELASTIC_PASSWORD" "$ELASTIC_HOST:$ELASTIC_PORT/$INDEX_NAME/_mapping/${INDEX_TO_COLLECTION_MAP[$INDEX_NAME]}?format=yaml" -H 'Content-Type: application/json' -d @$CONFIG_DIR/mapping_$INDEX_NAME.json
echo
